...
import { moduleMobileBrands } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [moduleMobileBrands],
    });

    await request(server)
      .get('/api/mobile-brands-entity-provider/todos')
      .expect(200, {
        items: [],
      });

    const createRes = await request(server)
      .post('/api/mobile-brands-entity-provider/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get(`/api/mobile-brands-entity-provider/todos/${createdTodoItem.id}`)
