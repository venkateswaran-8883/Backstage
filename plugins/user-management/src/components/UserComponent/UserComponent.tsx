...
export const UserComponent = () => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    const res = await fetchApi.fetch(
      'http://localhost:7007/api/user-management/users',
      {
        headers: {
  ...
  const createUser = async () => {
    ...
    await fetchApi.fetch('http://localhost:7007/api/user-management/users', {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
  ...
  const deleteUser = async (id?: number) => {
    ...
    await fetchApi.fetch(
      `http://localhost:7007/api/user-management/users/${id}`,
      {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
